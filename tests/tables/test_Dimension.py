...
class DimensionTest(unittest.TestCase):
    ...
    # The first help methods before the actual tests are (or can be) overridden in test classes for dimensions that
    # inherit from Dimension. Hence, the tests for Dimension can be reused by test classes for other dimensions
    ...
    # Returns a tuple consisting of: a part of a row (i.e. a dict containing a subset of the attributes)
    # and the number of occurrences of the part in the test dimension
    ...
    # Returns a part of a row that is not "contained" in any existing row in the test dimension
    def generate_part_of_row_that_is_nonexisting(self):
        return {
            "age": 99
        }

    # Returns a row that does not preexist in the test dimension.
    # The row is returned with or without the key depending on the withkey argument
    ...
    # Returns an updated version of a preexisting row together with the row's index in the table
    ...
    # This help method is not overridden anywhere but is needed:
    # The DTT format of a row cannot always be "hardcoded" since the content of the dict depends on what
    # is returned from other help methods
    ...
    # Not overridden but can be used to apply different namemappings for different schemas
    ...
    def test_lookup_with_namemapping(self):
        ...
        expected_key = row["id"]
        actual_key = self.test_dim.lookup(namemapped_row, namemapping=self.namemapping)
        self.cw.commit()
    ...
    def test_getbyvals_with_namemapping(self):
        ...
        # Determine the namemapping argument since vals_namemapped might only contain a subset of the attributes.
        # Hence, self.namemapping cannot be passed directly
        for key, value in self.namemapping.items():
            if value in vals_namemapped.keys():
                namemapping[key] = value

        rows = self.test_dim.getbyvals(vals_namemapped, namemapping=namemapping)
        self.cw.commit()
    ...
    def test_update_with_namemapping(self):
        ...
        self.test_dim.update(updated_row_namemapped, namemapping=self.namemapping)
        self.cw.commit()

        postcondition.assertEqual()

    # Update a non-existing row. The state of the database should remain unchanged.
    ...
    def test_ensure_with_namemapping(self):
        ...
        actual_key = self.test_dim.ensure(namemapped_row, namemapping=self.namemapping)
        self.cw.commit()
    ...
    def test_ensure_existing_row_with_namemapping(self):
        ...
        namemapped_existing_row = self.apply_namemapping(self.get_existing_row(withkey=True))

        expected_key = namemapped_existing_row["id"]
        actual_key = self.test_dim.ensure(namemapped_existing_row, namemapping=self.namemapping)
        self.cw.commit()
    ...
    def test_insert_one(self):
        postcondition = self.initial + self.convert_row_to_dtt_str(self.generate_nonexisting_row(withkey=True))
    ...
    def test_insert_with_namemapping(self):
        ...
        actual_key = self.test_dim.insert(new_row_namemapped, namemapping=self.namemapping)
        self.cw.commit()
    ...
    def test_idfinder(self):
        ...
        postcondition = self.initial + self.convert_row_to_dtt_str(row_with_dummy_key)
...
class CachedDimensionTest(DimensionTest):
    ...
    def test_prefill_false(self):
        ...
        # The following three rows should not have been cached, and an exception is raised since the connection is
        # closed
    ...
    def test_cachefullrows_false(self):
        ...
        # Full rows are not cached. Thus, looking up by key will raise an exception because the connection is closed
        for entire_row in [{"id": 1, "name": "Rufus", "city": "Hundeby", "age": 2},
                           {"id": 2, "name": "Rex", "city": "Vuffestad", "age": 5},
                           {"id": 3, "name": "King", "city": "Bjæfstrup", "age": 2}
                           ]:
        ...
        # However, the cache can still be used to lookup keys by lookup attributes
        for row, key_expected in [({"name": "Rufus", "city": "Hundeby", "age": 2}, 1),
                                  ({"name": "Rex", "city": "Vuffestad", "age": 5}, 2),
                                  ({"name": "King", "city": "Bjæfstrup", "age": 2}, 3)
                                  ]:
    ...
    def test_cacheoninsert_true(self):
        ...
        # Insert a number of new rows into the dimension (number of inserted rows is equal to the cache size). The
        # loop starts from 4 because the keys 1-3 are already in use
    ...
    def test_cacheoninsert_false(self):
        ...
        # The inserted rows should not have been cached -> exception raised because connection is closed
        for row in inserted_rows:
            self.assertRaises(Exception, self.test_dim.lookup, row)
    ...
    def test_size_custom_finite(self):
        ...
        # Only the rows inserted in the second round should still be in the cache (FIFO cache)
    ...
    def test_lookup_cache_initially_empty(self):
        ...
        # The cache should initially be empty because prefill is False and no lookups have been made
    ...
    def test_lookup_cache_after_some_lookups(self):
        ...
        self.assertRaises(Exception, self.test_dim.lookup, {"id": 3, "name": "King", "city": "Bjæfstrup", "age": 2})
    ...
    # Everything is cached since prefill and cacheonsinsert are true and as long the cache size has not been exceeded
    def test_update_caching_where_prefill_and_cacheoninsert_are_true_and_cache_size_is_not_exceeded(self):
        ...
        updated_row = {"id": 1, "name": "new_name", "city": "new_city", "age": 56}
        self.test_dim.update(updated_row)
        ...
        self.assertIsNone(self.test_dim.lookup({"name": "Rufus", "city": "Hundeby", "age": 2}))
    ...
    def test_update_caching_where_cacheoninsert_is_false(self):
        ...
        self.assertRaises(Exception, self.test_dim.lookup, {"name": "new_name", "city": "new_city", "age": 56})
        self.assertRaises(Exception, self.test_dim.lookup, {"name": "Rufus", "city": "Hundeby", "age": 2})
...
class SlowlyChangingDimensionTest(DimensionTest):
    ...
    # Returns a tuple consisting of a part of a row (a row containing a subset of the attributes) and the number of
    # occurrences of the part in the test dimension
    def get_part_of_row_and_num_of_occurrences(self):
        part_of_row = {
            "name": "Ann"
        }
    ...
    # Currently returns only two new rows
    def generate_multiple_nonexisting_rows(self):
        return [{"id": 5, "name": "Dan", "age": 45, "city": "Dublin", "fromdate": "2010-01-02",
                 "todate": "2010-03-04", "version": 1},
                {"id": 6, "name": "Eric", "age": 50, "city": "Ebeltoft", "fromdate": "2010-02-02",
                 "todate": "2010-02-02", "version": 1}
                ]
    ...
    def test_lookup_with_namemapping(self):
        ...
        expected_key = 3
        actual_key = self.test_dim.lookup(namemapped_row, namemapping={"name": "firstname"})
        self.cw.commit()
    ...
    def test_scdensure_existing_row(self):
        row = {'name': 'Ann', 'age': 20, 'city': 'Aarhus', 'from': '2010-03-03'}
        postcondition = self.initial
    ...
    def test_scdensure_namemapping(self):
        row = {'firstname': 'Ann', 'years': 20, 'town': 'Aarhus', 'from': '2010-03-03'}
        postcondition = self.initial
    ...
    def test_scdensure_newversion(self):
        row = {'name': 'Ann', 'age': 20, 'city': 'Aabenraa', 'from': '2010-04-04'}
        postcondition = self.initial.update(2, "| 3 | Ann | 20 | Aarhus | 2010-03-03 | 2010-04-04 | 2 |") \
    ...
    # The type1 attribute 'age' is changed - The 'age' attribute in all existing rows for Ann should be changed to 21
    def test_scdensure_type1_change_existing_row(self):
        ...
        self.test_dim.scdensure({'name': 'Ann', 'age': 21, 'city': 'Aarhus', 'from': '2010-03-03'})
        postcondition.assertEqual()

    # A new row should be inserted for Ann, and the attribute 'age' in all rows for Ann should be 21
    def test_scdensure_type1_change_new_row(self):
        ...
        self.test_dim.scdensure({'name': 'Ann', 'age': 21, 'city': 'Aabenraa', 'from': '2010-04-04'})
        postcondition.assertEqual()
    ...
    def test_scdensure_two_newversions(self):
        ...
        self.test_dim.scdensure({'name': 'Ann', 'age': 20, 'city': 'Aalborg', 'from': '2010-04-04'})
        self.test_dim.scdensure({'name': 'Ann', 'age': 20, 'city': 'Aabenraa', 'from': '2010-05-05'})

        postcondition.assertEqual()

    def test_scdensure_new_row(self):
        postcondition = self.initial + "| 5 | Doris | 85 | Dublin | 2010-04-04 | NULL | 1 |"
        self.test_dim.scdensure({'name': 'Doris', 'age': 85, 'city': 'Dublin', 'from': '2010-04-04'})
    ...
    # Tests that if both orderingatt and versionatt are defined, orderingatt is used to identify the newest version
    ...
    # A ValueError is raised since both the orderingatt, versionatt, and toatt are all None
    ...
    def test_minfrom(self):
        ...
        self.test_dim.scdensure({'name': 'Charlie', 'age': 24, 'city': 'Copenhagen'})

        postcondition = self.initial + "| 3 | Charlie | 24 | 2012-12-12 | Copenhagen | 1 |"
        postcondition.assertEqual()

    # minfrom should be ignored when inserting the row since it already contains a fromatt value
    def test_minfrom_is_ignored(self):
        ...
        self.test_dim.scdensure({'name': 'Charlie', 'age': 24, 'city': 'Copenhagen', 'fromdate': '2010-03-03'})

        postcondition = self.initial + "| 3 | Charlie | 24 | 2010-03-03 | Copenhagen | 1 |"
        postcondition.assertEqual()
    ...
    def test_maxto(self):
        ...
        self.test_dim.scdensure({'name': 'Ann', 'age': 20, 'city': 'Aarhus', 'from': '2012-12-12'})

        postcondition = self.initial.update(0, "| 1 | Ann | 20 | Aalborg | 2010-01-01 | 2012-12-12 | 1 |") \
    ...
    def test_idfinder(self):
        ...
        self.test_dim.scdensure({'name': 'Charlie', 'age': 30, 'city': 'Copenhagen'})
...
class SnowflakedDimensionTest(unittest.TestCase):
    def setUp(self):
        ...
        self.snowflaked_dim = SnowflakedDimension([(self.day_dim, self.month_dim), (self.month_dim, self.year_dim)])
    ...
    def test_lookup(self):
        ...
        self.assertEqual(1, self.snowflaked_dim.lookup({"Day": "January 1, 2000", "MonthID": 1}))
        self.assertEqual(32, self.snowflaked_dim.lookup({"Day": "February 1, 2000", "MonthID": 2}))
        self.assertEqual(33, self.snowflaked_dim.lookup({"Day": "February 2, 2000", "MonthID": 2}))
        self.assertEqual(366, self.snowflaked_dim.lookup({"Day": "January 1, 2001", "MonthID": 13}))
        self.assertEqual(731, self.snowflaked_dim.lookup({"Day": "January 1, 2002", "MonthID": 25}))
    ...
    def test_lookup_with_lookupatts(self):
        ...
        # Change the day dimension so it only uses the "Day" attribute as lookup attribute
        self.day_dim = Dimension(name=self.day_dt.name,
                                 key=self.day_dt.key(),
                                 attributes=self.day_dt.attributes,
                                 lookupatts=["Day"])
        ...
        self.assertEqual(1, self.snowflaked_dim.lookup({"Day": "January 1, 2000"}))
        self.assertEqual(32, self.snowflaked_dim.lookup({"Day": "February 1, 2000"}))
        self.assertEqual(33, self.snowflaked_dim.lookup({"Day": "February 2, 2000"}))
        self.assertEqual(366, self.snowflaked_dim.lookup({"Day": "January 1, 2001"}))
        self.assertEqual(731, self.snowflaked_dim.lookup({"Day": "January 1, 2002"}))
    ...
    def test_lookup_with_namemapping(self):
        ...
        self.assertEqual(1, self.snowflaked_dim.lookup({"Date": "January 1, 2000", "MonthID": 1}, namemapping=namemapping))
        self.assertEqual(32, self.snowflaked_dim.lookup({"Date": "February 1, 2000", "MonthID": 2}, namemapping=namemapping))
        self.assertEqual(33, self.snowflaked_dim.lookup({"Date": "February 2, 2000", "MonthID": 2}, namemapping=namemapping))
        self.assertEqual(366, self.snowflaked_dim.lookup({"Date": "January 1, 2001", "MonthID": 13}, namemapping=namemapping))
        self.assertEqual(731, self.snowflaked_dim.lookup({"Date": "January 1, 2002", "MonthID": 25}, namemapping=namemapping))
    ...
    def test_lookup_with_nonexisting_row(self):
        ...
        self.assertIsNone(self.snowflaked_dim.lookup({"Day": "January 45, 2099", "MonthID": 1}))
        self.assertIsNone(self.snowflaked_dim.lookup({"Day": "Non-existing row", "MonthID": -1}))
    ...
    def test_lookup_with_missing_atts(self):
        ...
        self.assertRaises(KeyError, self.snowflaked_dim.lookup, {"Day": "January 1, 2000"})
    ...
    def test_getbykey(self):
        ...
        self.assertDictEqual({"DayID": 1, "Day": "January 1, 2000", "MonthID": 1}, self.snowflaked_dim.getbykey(1))
        self.assertDictEqual({"DayID": 32, "Day": "February 1, 2000", "MonthID": 2}, self.snowflaked_dim.getbykey(32))
    ...
    def test_getbykey_nonexisting_row(self):
        ...
        self.assertDictEqual({"DayID": None, "Day": None, "MonthID": None}, self.snowflaked_dim.getbykey(99))
        self.assertDictEqual({"DayID": None, "Day": None, "MonthID": None}, self.snowflaked_dim.getbykey(-1))
    ...
    def test_getbykey_with_fullrow(self):
        ...
        expected_fullrow = {"DayID": 1, "Day": "January 1, 2000", "MonthID": 1, "Month": "January 2000",
                            "YearID": 1, "Year": 2000}

        self.assertDictEqual(expected_fullrow, self.snowflaked_dim.getbykey(1, fullrow=True))

        expected_fullrow = {"DayID": 731, "Day": "January 1, 2002", "MonthID": 25, "Month": "January 2002",
                            "YearID": 3, "Year": 2002}

        self.assertDictEqual(expected_fullrow, self.snowflaked_dim.getbykey(731, fullrow=True))
    ...
    def test_getbyvals_with_fullrow(self):
        ...
        vals = {"MonthID": 2}
        expected_rows = [
            {"DayID": 32, "Day": "February 1, 2000", "MonthID": 2, "Month": "February 2000", "YearID": 1, "Year": 2000},
            {"DayID": 33, "Day": "February 2, 2000", "MonthID": 2, "Month": "February 2000", "YearID": 1, "Year": 2000},
        ]
    ...
    def test_getbyvals_with_namemapping(self):
        ...
        actual_rows = self.snowflaked_dim.getbyvals(vals, fullrow=False, namemapping=namemapping)
    ...
    def test_update_without_foreign_keys(self):
        postcondition_day = self.day_dt.update(2, "| 33 | February 2nd in year 2000 | 2 |")
        postcondition_month = self.month_dt
        postcondition_year = self.year_dt
    ...
    # The attributes from the referenced table 'Month' that are present in the row should also be updated
    def test_update_with_foreign_key_and_with_foreign_nonkey_atts_present_in_row(self):
        postcondition_day = self.day_dt.update(2, "| 33 | February 2nd in year 2000 | 2 |")
        postcondition_month = self.month_dt.update(1, "| 2 | February in year 2000 | 1 |")
        postcondition_year = self.year_dt

        updated_row = {"DayID": 33, "MonthID": 2, "Day": "February 2nd in year 2000", "Month": "February in year 2000"}
        self.snowflaked_dim.update(updated_row)
    ...
    def test_update_with_foreign_key_and_but_without_foreign_atts_present_in_row(self):
        ...
        updated_row = {"DayID": 33, "MonthID": 2, "Day": "February 2nd in year 2000"}
        self.snowflaked_dim.update(updated_row)
    ...
    def test_update_do_nothing(self):
        ...
        self.snowflaked_dim.update({"Day": "Some day", "Month": "Some month", "Year": "Some year"})
    ...
    def test_ensure_only_new_row_in_root(self):
        ...
        self.snowflaked_dim.ensure({"DayID": 5, "Day": "January 5, 2000", "Month": "January 2000",
                                    "Year": 2000})
    ...
    def test_ensure_new_row_in_all_dimensions(self):
        ...
        self.snowflaked_dim.ensure({"DayID": 1200, "Day": "March 3, 2003", "MonthID": 39, "Month": "March 2003",
                                    "Year": 2003})
    ...
    def test_ensure_two_new_rows_in_all_dimensions(self):
        postcondition_day = self.day_dt + "| 1200 | March 3, 2003  | 39 |" + "| 1700 | April 4, 2004  | 52 |"
        postcondition_month = self.month_dt + "| 39 | March 2003 | 4 |" + "| 52 | April 2004 | 5 |"
        postcondition_year = self.year_dt + "| 4 | 2003 |" + "| 5 | 2004 |"

        self.snowflaked_dim.ensure({"DayID": 1200, "Day": "March 3, 2003", "MonthID": 39, "Month": "March 2003",
                                    "Year": 2003})
        self.snowflaked_dim.ensure({"DayID": 1700, "Day": "April 4, 2004", "MonthID": 52, "Month": "April 2004",
                                    "Year": 2004})
    ...
    def test_ensure_existing_row(self):
        ...
        self.snowflaked_dim.ensure({"Day": "January 1, 2000", "Month": "January 2000", "Year": 2000})
    ...
    def test_ensure_nonexisting_row_with_namemapping(self):
        ...
        self.snowflaked_dim.ensure({"DayID": 5, "Date": "January 5, 2000", "Month": "January 2000",
                                    "Year": 2000}, namemapping=namemapping)
    ...
    def test_ensure_existing_row_with_namemapping(self):
        ...
        self.snowflaked_dim.ensure({"Date": "January 1, 2000", "Month": "January 2000", "Year": 2000},
    ...
    def test_insert_only_new_row_in_root(self):
        ...
        self.snowflaked_dim.insert({"DayID": 5, "Day": "January 5, 2000", "Month": "January 2000",
                                    "Year": 2000})
    ...
    def test_insert_new_row_in_all_dimensions(self):
        ...
        self.snowflaked_dim.insert({"DayID": 1200, "Day": "March 3, 2003", "MonthID": 39, "Month": "March 2003",
                                    "Year": 2003})
    ...
    def test_insert_two_new_rows_in_all_dimensions(self):
        ...
        self.snowflaked_dim.insert({"DayID": 1200, "Day": "March 3, 2003", "MonthID": 39, "Month": "March 2003",
                                    "Year": 2003})
        self.snowflaked_dim.insert({"DayID": 1700, "Day": "April 4, 2004", "MonthID": 52, "Month": "April 2004",
                                    "Year": 2004})
    ...
    def test_insert_new_row_with_namemapping(self):
        ...
        self.snowflaked_dim.insert({"DayID": 5, "Date": "January 5, 2000", "Month": "January 2000",
                                    "Year": 2000}, namemapping=namemapping)
